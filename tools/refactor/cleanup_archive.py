...
def main():
    archive_dir = "g:\\Mi unidad\\ia\\luna_v2\\data\\archive"
    print(f"[CLEANUP-START] Iniciando limpieza segura de matrices de entrenamiento en: {archive_dir}")
    sys.stdout.flush()
    
    if not os.path.exists(archive_dir):
        print(f"[CLEANUP-ERROR] La ruta del archivo no existe: {archive_dir}")
        return
        
    total_files_deleted = 0
    total_bytes_reclaimed = 0
    
    # Recorremos la carpeta archive
            ...
            if f.startswith("features_train") and f.endswith(".parquet"):
                ...
                if not os.path.islink(fp):
                    try:
                        ...
                        # Cada 100 archivos borrados imprimimos progreso en los logs
                        if total_files_deleted % 100 == 0:
                            mb_reclaimed = round(total_bytes_reclaimed / (1024 * 1024), 2)
                            print(f"[CLEANUP-PROGRESS] Borrados {total_files_deleted} archivos... {mb_reclaimed} MB liberados.")
                            sys.stdout.flush()
                    except OSError as e:
                        print(f"[CLEANUP-WARN] No se pudo eliminar {fp}: {e}")
                        sys.stdout.flush()
    ...
    print("\n" + "=" * 80)
    print(f"      [CLEANUP-SUCCESS] LIMPIEZA DE ARCHIVO COMPLETADA CON EXITO!")
