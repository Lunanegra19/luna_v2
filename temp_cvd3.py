...
FUENTE: data/predictions/oos_trades_W*_seed*.parquet
...
if _DATA_AUTO.exists() and len(list(_DATA_AUTO.glob('oos_trades_W*_seed*.parquet'))) > 0:
    DATA = _DATA_AUTO
    print(f"[FIX-CVD-PATH-01] Ruta local auto-detectada: {DATA}")
elif _FALLBACK_G.exists() and len(list(_FALLBACK_G.glob('oos_trades_W*_seed*.parquet'))) > 0:
    DATA = _FALLBACK_G
...
for f in sorted(DATA.glob('oos_trades_W*_seed*.parquet')):
    ...
    d['_seed'] = int(f.stem.split('seed')[1].split('_')[0])
    d['_window'] = f.stem.split('W')[1].split('_')[0]
    dfs.append(d)
...
df['_window'] = 'W' + df['_window'].astype(str)
