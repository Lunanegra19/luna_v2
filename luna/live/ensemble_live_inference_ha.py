...
class LunaEnsembleLiveInference:
    ...
    def _load_all_seeds(self):
        ...
        # [H-A 2026-06-12] Anti-Overfitting Pruning
        try:
            max_dsr = float(getattr(self.settings.wfb, "ensemble_max_is_dsr", 0.0))
        except Exception:
            max_dsr = 0.0
        ...
        for seed in self.active_seeds:
            ...
            # [H-A] Leer firma para aplicar podado Inverse Sharpe
            sig_path = seed_dir / "xgboost_meta_1_BULL_TREND_long_signature.json"
            if not sig_path.exists():
                sig_path = seed_dir / "xgboost_meta_bull_long_signature.json"
            ...
            if sig_path.exists():
                try:
                    ...
                    if dsr_is > max_dsr:
                        pruned_count += 1
                        logger.info(f"[EnsembleLive] PRUNING (Anti-Overfitting): Semilla {seed} descartada (DSR {dsr_is:.3f} > {max_dsr})")
                        print(f"[EnsembleLive/PRUNING] Semilla {seed} descartada por overfitting (DSR {dsr_is:.3f} > {max_dsr}).")
                        continue
                except Exception:
                    pass
        ...
        self.active_seeds = valid_seeds
        if pruned_count > 0:
            print(f"[EnsembleLive/PRUNING] Total semillas activas post-pruning: {len(self.active_seeds)} (Se podaron {pruned_count}).")
            logger.info(f"[EnsembleLive] Semillas activas actualizadas a {len(self.active_seeds)} tras podar {pruned_count} overfitters.")
