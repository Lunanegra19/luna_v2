...
- 🔒 **BASE DETERMINISTA (PUNTO DE RETORNO) — tag git `determinismo-base-20260626` (commit `c318121`, 2026-06-26).** El pipeline WFB es REPRODUCIBLE desde aquí: dos runs frescas `--nocache` de la misma seed dan resultados byte-idénticos en el núcleo (AE/features/C-MI). 8 fuentes de no-determinismo selladas (`[FIX-AE-DETERMINISM-01]`) + **test guardián** `scripts/pre_flight/test_determinism.py` (DET-1..7) que corre al inicio de cada WFB y alerta `*** DETERMINISMO ROTO ***` si un fix futuro lo desalinea. **Esta es la base limpia sobre la que se hacen las pruebas de palancas; para revertir a este punto: `git checkout determinismo-base-20260626`.** Detalle completo: `docs/hallazgos_run_baseline_20260626.md` §11. (Residual conocido y aceptado: 1 feature on-chain correlacionado en la selección del SFI, efecto despreciable.)
